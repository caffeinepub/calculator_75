...
function App() {
  ...
  const [operator, setOperator] = reactExports.useState(null);
  const [prevValue, setPrevValue] = reactExports.useState(null);
  const [justEvaluated, setJustEvaluated] = reactExports.useState(false);
  const [pressedKey, setPressedKey] = reactExports.useState(null);
  const btnRefs = reactExports.useRef({});
  const flashButton = reactExports.useCallback((ocid) => {
    setPressedKey(ocid);
  ...
  const inputDecimal = reactExports.useCallback(() => {
    setDisplay((prev) => {
      if (waitingForOperand || justEvaluated) {
        setWaitingForOperand(false);
        setJustEvaluated(false);
  ...
  const percentage = reactExports.useCallback(() => {
    setDisplay((prev) => {
  ...
  const handleOperator = reactExports.useCallback(
    (nextOp) => {
      ...
      if (prevValue !== null && !waitingForOperand && !justEvaluated) {
        ...
        const resultStr = Number.isInteger(result) ? String(result) : String(Number.parseFloat(result.toPrecision(12)));
  ...
  const evaluate = reactExports.useCallback(() => {
    const current = Number.parseFloat(display);
    ...
    const expr = expression.trimEnd().endsWith(operator) ? `${expression} ${display} =` : `${expression} =`;
    setExpression(expr);
    const resultStr = Number.isInteger(result) ? String(result) : String(Number.parseFloat(result.toPrecision(12)));
    setDisplay(resultStr);
    setPrevValue(null);
  ...
  const variantStyles = {
    digit: "bg-calc-digit hover:bg-calc-digit-hover text-foreground shadow-btn-digit",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        ...
        children: [
              ...
              children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "div",
                  {
                    ...
                    style: {
                      color: display === "Error" ? "oklch(0.62 0.17 22)" : "oklch(0.97 0.005 80)"
                    },
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "p-4 grid grid-cols-4 gap-3", children: buttons.map((btn) => /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              type: "button",
              "data-ocid": btn.ocid,
              ...
              className: [
                ...
                btn.wide ? "col-span-2" : "",
                pressedKey === btn.ocid ? "translate-y-px shadow-none brightness-90" : ""
              ].join(" "),
              children: btn.label
            },
            btn.ocid
          )) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-4 pb-4 text-center", children: /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs", style: { color: "oklch(0.60 0.02 70)" }, children: "Keyboard supported · Esc to clear · Enter to evaluate" }) })
        ]
      }
    ),
    /* @__PURE__ */ jsxRuntimeExports.jsx("footer", { className: "mt-8 text-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-xs", style: { color: "oklch(0.60 0.015 70)" }, children: [
      "© ",
